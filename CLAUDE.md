...
4. **Refresh the marketplace and update the plugin**:
   - `claude plugin marketplace update dansunotori` — refreshes the local marketplace catalogue from git remote
   - `claude plugin update <name>@dansunotori` — downloads the new version into the cache
...
Plugin versions are resolved from the first available source: `plugin.json` `version` field → marketplace.json `version` field → git commit SHA. Setting an explicit version pins the plugin — new commits without a version bump will NOT trigger updates. The version in `plugin.json` takes precedence over `marketplace.json`.
...
This project is indexed by GitNexus as **dansunotori-plugins** (167 symbols, 162 relationships, 0 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/dansunotori-plugins/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/dansunotori-plugins/context` | Codebase overview, check index freshness |
| `gitnexus://repo/dansunotori-plugins/clusters` | All functional areas |
| `gitnexus://repo/dansunotori-plugins/processes` | All execution flows |
| `gitnexus://repo/dansunotori-plugins/process/{name}` | Step-by-step execution trace |
